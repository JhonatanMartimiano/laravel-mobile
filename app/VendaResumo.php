...
class VendaResumo extends Model
{
    private $headers;
    /**
    ...
    public function makeDataTable($data)
    {
            ...
            'printHeader' => "
                    ...
                    <header class='header-print'>
                        <p>
                            ...
                            <b>Tipo do relatório:</b> Agrupado por " . ucfirst($this->filtros['tipo_relatorio']) . "<br>
        ...
        $table = "<div class='table-responsive'>
                <table id='rp-table' class='table table-striped table-bordered'>
                    ...
                    <tr>
                        ...
                        <th style='min-width: 200px!important;'>Quantidade de transações</th>
                        <th>Total</th>
                    </tr>
                    </thead>
                    <tbody>";
        ...
        foreach ($data as $key => $dt):
            if ($this->isFiltroValidado($dt)):
                ...
                $dataOk[$dateTime->format('d/m/Y')][ucwords(mb_strtolower($dt['BandeiraNome']))][] = [
                    'rede' => ucwords(mb_strtolower($dt['RedeNome'])),
                    'servico' => ucwords(mb_strtolower($dt['ServicoDescricao'])),
                    'valor' => $dt['TransacaoValor'],
                ];
            endif;
        endforeach;

        /* Monta tabela para retorno */
        $total = 0;
        foreach ($dataOk as $dia => $infosBandeira):
            foreach ($infosBandeira as $bandeira => $item):
                $print = ['qtd' => 0, 'total' => 0];
                foreach ($item as $dados):
                    $print['qtd']++;
                    $print['total'] += $dados['valor'];
                    $print['rede'] = $dados['rede'];
                    $print['servico'] = $dados['servico'];
                endforeach;
                ...
                $table .= "<tr class='font-13'>
                        ...
                        <td>{$print['servico']}</td>
                        <td>{$print['qtd']}</td>
                        <td>R$" . number_format($print['total'], 2, ',', '.') . "</td>
                    </tr>
                    ";
